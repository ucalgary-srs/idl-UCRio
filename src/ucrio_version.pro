...
;         Indicate that the function is being run during the IDL-UCRio initialization phase
;
; :Returns:
;       Struct
...
;       ucrio_check_version
;+
function ucrio_check_version, quiet = quiet, init_mode = init_mode
  ; init
  quiet_flag = 0
  print_prefix = 'ucrio_check_version'
  if (keyword_set(quiet)) then quiet_flag = 1
  if (keyword_set(init_mode)) then print_prefix = 'idl-ucrio'

  ; get current version
  curr_version = __ucrio_version()

  ; make request to Github to get latest release from the idl-ucrio repository
  req = obj_new('IDLnetUrl')
  req.setProperty, url_scheme = 'https'
  req.setProperty, url_port = 443
  req.setProperty, url_host = 'api.github.com'
  req.setProperty, url_path = 'repos/ucalgary-srs/idl-ucrio/releases/latest'
  ...
  if (new_version_available eq 0) then begin
    message = '[' + print_prefix + '] No new version is available. Your version, ' + curr_version + ', is the ' + $
      'latest. More information can be found at https://github.com/ucalgary-srs/idl-ucrio/releases'
    if (quiet_flag ne 1) then begin
      print, message
    endif
  endif else begin
    message = '[' + print_prefix + '] New version is available! Version ' + latest_version + ' can be installed (' + $
      ...
      'https://github.com/ucalgary-srs/idl-ucrio?tab=readme-ov-file#updating'
    if (quiet_flag ne 1) then begin
      print, message
    endif
  endelse
