...
;
; :Examples:
;       ucrio_map_oplot, point=[245,61.2], color=ucrio_get_decomposed_color([0,0,255])
;       ucrio_map_oplot, constant_lats=[40,50,60], constant_lons=[220,240,260], linestyle=2, thick=3
;+
pro ucrio_map_oplot, $
  ...
  if not isa(color, /scalar, /number) or color lt 0 or color gt ucrio_get_decomposed_color([255, 255, 255]) then begin
    print, '[ucrio_map_plot_contour] Error: ''color'' must be a scalar specifying a valid ' + $
      'long integer decomposed color. Use ucrio_get_decomposed_color to obtain color ' + $
      'integer from RGB triple.'
    goto, error
  endif
  if not isa(thick, /scalar, /number) or thick lt 0 then begin
    print, '[ucrio_map_plot_contour] Error: ''thick'' must be a positive integer ' + $
      'specifying the contour thickness.'
    goto, error
  endif
  if not isa(symsize, /scalar, /number) or symsize lt 0 then begin
    print, '[ucrio_map_plot_contour] Error: ''symsize'' must be a positive integer.'
    goto, error
  endif
  if not isa(linestyle, /scalar, /number) or linestyle lt 0 or linestyle gt 6 then begin
    print, '[ucrio_map_plot_contour] Error: ''linestyle'' must be an integer ' + $
      'from 0-6. (See IDL built-in linestyles).'
    goto, error
  endif
  if not isa(symbol, /scalar, /number) or linestyle lt 0 or linestyle gt 6 then begin
    print, '[ucrio_map_plot_contour] Error: ''symbol'' must be an integer ' + $
  ...
  if keyword_set(aacgm_date) then begin
    if ~isa(aacgm_date, /string, /scalar) then begin
      print, '[ucrio_map_plot_contour] Error: keyword ''aacgm_date'' should be a scalar string in the format "YYYY-MM-DD"'
      goto, error
    endif
  ...
  if not keyword_set(constant_lons) and not keyword_set(constant_lats) and not keyword_set(point) then begin
    print, '[ucrio_map_plot_contour] Error: At least one of ''constant_lons'' ' + $
  ...
  if keyword_set(constant_lons) and not isa(constant_lons, /number) then begin
    print, '[ucrio_map_plot_contour] Error: ''constant_lons'' must be a number ' + $
  ...
  if keyword_set(constant_lats) and not isa(constant_lats, /number) then begin
    print, '[ucrio_map_plot_contour] Error: ''constant_lats'' must be a number ' + $
      'or an array of numbers.'
    goto, error
  endif
  ...
  if keyword_set(point) and not isa(point, /array, /number) then begin
    print, '[ucrio_map_plot_contour] Error: ''point'' must be a 2-element ' + $
