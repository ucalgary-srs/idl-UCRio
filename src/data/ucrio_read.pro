...
function ucrio_read, dataset, file_list, start_dt = start_dt, end_dt = end_dt, first_record = first_record, no_metadata = no_metadata, quiet = quiet
  ...
  if (read_function eq 'norstar') then begin
    if (quiet_flag eq 0) then begin
      __ucrio_readfile_norstar, file_list, data, meta, start_dt = start_dt, end_dt = end_dt, first_frame = first_record, no_metadata = no_metadata, /verbose, /show_datarate
    endif else begin
      __ucrio_readfile_hsr, file_list, data, meta, start_dt = start_dt, end_dt = end_dt, first_frame = first_record, no_metadata = no_metadata
    endelse
  endif
