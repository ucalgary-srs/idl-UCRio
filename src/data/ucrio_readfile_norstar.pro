...
pro __ucrio_readfile_norstar, $
  ...
  foreach f, filenames, file_num do begin
    ...
    file_julday = julday(fix(strmid(ymd,4,2)), fix(strmid(ymd,6,2)), fix(strmid(ymd,0,4)))
    ...
    if keyword_set(start_dt) or keyword_set(end_dt) then begin
      if keyword_set(start_dt) then begin
        ...
        start_julday = julday(fix(start_mm), fix(start_dd), fix(start_yy), 0, 0, 0)
      ...
      if keyword_set(end_dt) then begin
        ...
        end_julday = julday(fix(end_mm), fix(end_dd), fix(end_yy), 0, 0, 0)
    ...
    if (where(file_metadata.keys() eq 'site_unique_id', /null) eq !null) then begin
      if ~ keyword_set(quiet) then print, '[ucrio_read] Warning: Unable to determine "site_unique_id" field in file '+f
