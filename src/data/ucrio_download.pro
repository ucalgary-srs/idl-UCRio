...
function __get_download_path
  ...
  download_path = (file_search('~', /expand_tilde))[0] + path_sep() + 'idlucrio_data'
...
;         unique device UID to filter on (e.g., hsr-13)
...
;
; :Examples:
;       d = ucrio_download('NORSTAR_RIOMETER_K2_TXT', '2022-01-01T00:00', '2022-01-01T23:59', site_uid='chur')
;       d = ucrio_download('SWAN_HSR_K0_H5', '2025-01-01T00:00', '2025-01-01T00:00', /overwrite)
;+
function ucrio_download, $
  dataset_name, $
  start_ts, $
  end_ts, $
  site_uid = site_uid, $
  ...
  ; get urls
  urls = ucrio_get_urls(dataset_name, start_ts, end_ts, site_uid = site_uid, device_uid = device_uid)

  ; download each file
  if (n_elements(urls.urls) eq 0) then begin
    if (quiet_flag eq 0) then print, '[ucrio_download] No files found to download'
    ...
    for i = 0, n_elements(urls.urls) - 1 do begin
      ...
      if (overwrite_flag eq 0 and file_test(output_filename) eq 1) then begin
        if (quiet_flag eq 0) then print, '[ucrio_download] File already exists, not redownloading (' + output_filename + ')'
      ...
      if (error_status ne 0) then begin
        ...
        if (quiet_flag eq 0) then print, '[ucrio_download] URL download failed with error code ' + strtrim(string(rspCode), 2) + ': ' + url
      ...
      if (quiet_flag eq 0) then print, '[ucrio_download] Successfully downloaded ' + url
  ...
  if (quiet_flag eq 0) then begin
    ...
    endwhile
    prefix = (['', 'K', 'M', 'G', 'T'])[i]
    print, '[ucrio_download] Finished, downloaded ' + strcompress(fix(working_total_bytes), /remove_all) + ' ' + prefix + 'B in ' + strcompress(string(dtime, format = '(d20.2)'), /remove_all) + ' seconds'
  endif
