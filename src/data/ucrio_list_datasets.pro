...
;       datasets = ucrio_list_datasets(name='hsr')
;+
function ucrio_list_datasets, name = name, level = level
  ; set params
  param_str = '?'
  ...
  ; make request
  r = __ucrio_perform_api_request('get', 'ucrio_list_datasets', req)
  ...
  for i = 0, n_elements(status) - 1 do begin
    if (status[i].supported_libraries.where('idl-ucrio') ne !null) then begin
